    ...
    def set_input(self, input):
        ...
        if self.opt.isTrain:
            ...
            for i,im in enumerate(self.image_paths):
                self.id_[int(im.replace('\\','/').split('/')[-1].split('.png')[0].split('_')[-1])]=i
                self.obs += [int(im.replace('\\','/').split('/')[-1].split('.png')[0].split('_')[-1])]
                # self.id_[im.replace('\\', '/').split('/')[-1].split('.png')[0].split('_')[-1].split(' ')[0]] = i
                # self.obs += [im.replace('\\', '/').split('/')[-1].split('.png')[0].split('_')[-1].split(' ')[0]]
    ...
    def forward1(self, inp_grad=False):
        ...
        for batch in range(self.opt.output_nc):
            if not self.opt.rgb_in and self.opt.rgb_out:
                ...
                # real_A1[batch, :, :, :] = inp_orna.data[batch, self.out_id[batch] * self.opt.input_nc_1:(self.out_id[batch] + 1) * self.opt.input_nc_1, :, :]
                # temp = inp_orna.data[0, self.out_id[batch]*np.array(self.opt.input_nc_1):(self.out_id[batch]+1)*np.array(self.opt.input_nc_1),:,:]
                # temp = inp_orna.data[0,
                #        self.out_id[batch] * self.opt.input_nc_1:(self.out_id[batch] + 1) *
                #            self.opt.input_nc_1, :, :]
                real_A1[batch, 0, :, :] = inp_orna.data[self.id_[batch], batch, :, :]
                real_A1[batch, 1, :, :] = inp_orna.data[self.id_[batch], batch, :, :]
                real_A1[batch, 2, :, :] = inp_orna.data[self.id_[batch], batch, :, :]
                # real_A1[batch, :, :, :] = inp_orna.data[self.id_[batch], batch, :, :]
        if self.initial:
